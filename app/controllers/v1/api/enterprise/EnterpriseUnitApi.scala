package controllers.v1.api.enterprise
...
trait EnterpriseUnitApi {
  @ApiOperation(
    ...
    response = classOf[examples.EnterpriseLinkedUnitForSwagger],
    code = 200,
    httpMethod = "GET"
  )
  @ApiResponses(Array(
...
package examples {
  ...
  case class EnterpriseLinkedUnitForSwagger(
    ...
    @ApiModelProperty(value = "the identifiers of child units along with the associated unit type", dataType = "controllers.v1.api.enterprise.examples.EnterpriseChildrenForSwagger", required = false) children: Option[Map[UnitId, UnitType]],
