 ...
 */
class LocalUnitRoutingSpec extends FreeSpec with Matchers with GuiceOneAppPerSuite with ScalaFutures {
  ...
  "A request to retrieve a local unit by Local Unit reference (LURN) and period" - {
    "is rejected when" - {
      "the LURN" - {
        "has fewer than nine digits" in new Fixture {
          val LurnTooFewDigits = ValidLurn.drop(1)

          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$ValidPeriod/lous/$LurnTooFewDigits"))
        ...
        "has more than nine digits" in new Fixture {
          ...
          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$ValidPeriod/lous/$LurnTooManyDigits"))

          status(result) shouldBe BAD_REQUEST
        }

        "is non-numeric" in new Fixture {
          val LurnNonNumeric = new String(Array.fill(9)('A'))

          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$ValidPeriod/lous/$LurnNonNumeric"))
      ...
      "the Period" - {
        "has fewer than six digits" in new Fixture {
          ...
          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$PeriodTooFewDigits/lous/$ValidLurn"))
        ...
        "has more than six digits" in new Fixture {
          ...
          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$PeriodTooManyDigits/lous/$ValidLurn"))
        ...
        "is non-numeric" in new Fixture {
          ...
          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$PeriodNonNumeric/lous/$ValidLurn"))
        ...
        "is negative" in new Fixture {
          ...
          val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$PeriodNegative/lous/$ValidLurn"))
        ...
        "has an invalid month value" - {
          "that is too low" in new Fixture {
            ...
            val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$PeriodInvalidBeforeCalendarMonth/lous/$ValidLurn"))
          ...
          "that is too high" in new Fixture {
            ...
            val Some(result) = route(app, FakeRequest(GET, s"/v1/periods/$PeriodInvalidAfterCalendarMonth/lous/$ValidLurn"))
    ...
    "is processed when valid" in new Fixture {
      ...
      Months.foreach { month =>
        withClue(s"for month $month") {
          ...
          val Some(futureResult) = route(app, FakeRequest(GET, s"/v1/periods/$validPeriod/lous/$ValidLurn"))
